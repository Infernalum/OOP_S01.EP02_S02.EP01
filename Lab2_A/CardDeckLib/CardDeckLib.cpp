...
namespace CardDeckClass {
	...
	CardDeck& CardDeck::DefaultSort() {
		...
		for (int i = 49; i < 53; i++) {
			if (suit_beginning == GetCurrentSize())
				return *this;
			while (suit_ends < GetCurrentSize() && GetCurrentSuit(suit_ends + 1) == i)
				suit_ends++;
	...
	CardDeck& CardDeck::GetSuitDeck(int s) const {
		...
		for (int i = 0; i < GetCurrentSize(); ++i) {
			if (!GetCurrentSuit(i + 1))
				return res;
			if (GetCurrentSuit(i + 1) == (s + 48))
				res.cards[j++] = cards[i];
		}
		return res;
	}
