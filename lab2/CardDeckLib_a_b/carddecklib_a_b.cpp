...
#include "carddecklib_a_b.h"
#include <stdio.h>
#include <ctime>
#include <stdexcept>

const char* Rangs[] = {"Two",   "Three", "Four", "Five", "Six",
...
CardDeck::CardDeck(int n) {
  if (n > 1 && n <= SZ) {
    ...
    throw std::invalid_argument(
        "Неверное количество карт в колоде. Попробуйте еще раз");
}
...
// Поиск конкретной карты в колоде; в случае успеха возвращает индекс массива с
// этой картой, иначе -1
int CardDeck::FindCurrentCard(int r, int s) const {
  for (int i = 0; i < SZ; ++i) {
    // Т.к. все карты располагаются в начале, если доходим до пустой ячейки,
    // заканчивем поиск с -1
  ...
  for (int i = 0; i < size; ++i) {
    ...
    if (rc == 1)
      ...
      if (deck.FindCurrentCard(rang + 47, suit + 48) != -1) {
        throw std::invalid_argument(
            "Такая карта уже существует в колоде. Попробуйте заново.");
      } else {
...
std::ostream& operator<<(std::ostream& stream, const CardDeck& deck) {
  if (!deck.GetCurrentSize())
    ...
    for (int i = 0; i < deck.GetCurrentSize(); ++i) {
      stream << i + 1 << ": " << Rangs[deck[i + 1] - 49] << ' '
             << Suits[deck(i + 1) - 49] << ' ' << std::endl;
    }
  }
  return stream;
}
