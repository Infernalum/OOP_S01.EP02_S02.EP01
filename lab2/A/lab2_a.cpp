...
const char* Rang[] = {"Двойка",  "Тройка",    "Четвёрка", "Пятёрка", "Шестёрка",
                      "Семёрка", "Восьмёрка", "Девятка",  "Десятка", "Валет",
                      "Дама",    "Король",    "Туз"};
...
int D_GetCurrentCard(CardDeckClass::CardDeck& deck) {
  try {
    ...
    int suit = deck.GetCurrentSuit(card);
    int rang = deck.GetCurrentRang(card);
    qtConsole("Ваша карта: ");
    qtConsole(Rang[rang - 49]);
    std::cout << ' ';
    qtConsole(Suit[suit - 49]);
    std::cout << std::endl;
  } catch (const std::exception& ex) {
    qtConsole(ex.what());
    std::cout << std::endl;
