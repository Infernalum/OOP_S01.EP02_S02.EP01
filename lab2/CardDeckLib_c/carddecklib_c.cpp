...
const char* CardDeck::Rangs[13] = {"Two",   "Three", "Four", "Five", "Six",
                                   "Seven", "Eight", "Nine", "Ten",  "Jack",
                                   "Queen", "King",  "Joker"};

const char* CardDeck::Suits[] = {"Diamonds", "Hearts", "Spades", "Clubs"};
...
/* Инициализирующий конструктор с заданием количества карт, масть и ранг которых
определяются случайно Выбрасывается исключение, если n > SZ */
CardDeck::CardDeck(int size, int n) {
  if (size < n)
    throw std::invalid_argument(
        "Кол-во инициализирующихся карт больше максимального размера колоды. "
        "Попрообуйте еще раз.");
  SZ = size;
  cards = new Card[SZ];
  srand(time(NULL));
...
Выбрасывается исключение, если n = 0, либо масть или ранг карты не лежит в
нужных интервалах */
CardDeck::CardDeck(int n, int r, int s) {
  if (!n)
    throw std::invalid_argument(
        "Начальный размер колоды не может быть пустым. Попробуйте еще раз.");
  if (r > 61 || r < 49 || s > 52 || s < 49)
    throw std::invalid_argument(
        "Введены неверный ранг или масть инициализирующей карты. Попробуйте "
        "еще раз. ");
  SZ = n;
  cards = new Card[SZ];
...
// Сортировка деки по условию лабы (по масти: Бубе -> Черви -> Пики -> Крести, и
// по убыванию рангу)
CardDeck& CardDeck::DefaultSort() {
  ...
  for (int i = 49; i < 53; i++) {
    ...
    while (suit_ends < GetCurrentSize() && (*this)(suit_ends + 1) == i)
      suit_ends++;
    qsort(this->cards + suit_beginning, suit_ends - suit_beginning,
          sizeof(Card), CompareRang);
...
int CardDeck::GetCurrentSuit(int i) const {
  if (i < 1 || i > GetCurrentSize())
    throw std::out_of_range(
        "Неправильный номер карты (либо больший, либо меньший возможного)");
...
CardDeck& CardDeck::GetSuitDeck(int s) const {
  if (s < 1 || s > 4)
    throw std::out_of_range("Пожалуйста, выберите существующий ранг: --> ");
...
std::istream& operator>>(std::istream& stream, CardDeck& deck) {
  ...
  if (size > deck.SZ || size < 1)
    throw std::out_of_range(
        "Некорректное число карт (либо больше, либо меньше возможного). "
        "Попробуйте еще раз.");
  for (int i = 0; i < size; ++i) {
    int rc = 0;
    if (getNum(rc, stream))
      throw std::out_of_range(
          "Было введено неправильное число. Попробуйте еще раз");
    while (rc != 1 && rc != 2) {
      if (getNum(rc, stream))
        throw std::out_of_range(
            "Было введено неправильное число. Попробуйте еще раз");
...
std::ostream& operator<<(std::ostream& stream, const CardDeck& deck) {
  if (!deck.GetCurrentSize())
    ...
    stream << "Маскимальный размер колоды на данный момент: "
           << deck.GetMaxSize() << '\n'
           << "Колода:\n";
    for (int i = 0; i < deck.GetCurrentSize(); ++i) {
      stream << i + 1 << ": " << CardDeck::Rangs[deck[i + 1] - 49] << ' '
             << CardDeck::Suits[deck(i + 1) - 49] << ' ' << std::endl;
    }
  }
  return stream;
}
