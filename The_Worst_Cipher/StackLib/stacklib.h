...
// Шаблон функции проверки ввода любого типа данных (1 в случае неудачи, и 0 при
// успехе)
template <class T>
...
struct elem_of_stack {
 ...
 public:
  ...
  elem_of_stack& operator=(int k) { return set_key(k); }
  elem_of_stack& operator=(const std::string& str) { return set_string(str); }
  elem_of_stack& operator+=(int k) { return set_key(key += k); }
...
class Stack {
 ...
 public:
  Stack() : size(SZ), top(0) { mas = new elem_of_stack[SZ]; };
  ...
  friend std::ostream& operator<<(std::ostream& c,
                                  Stack& a);  //выводит весь стек в поток
  friend std::istream& operator>>(std::istream&,
                                  Stack&);  //добавляет элемент из потока
  elem_of_stack& show_first() const;  //печать первого элемента
  Stack& operator()(elem_of_stack&);  //выборка из стека pop
  int empty() const;  //проверяет стек на заполненность(-1 частично заполнен; 0
                      //пустой; 1 полный)
};
}  // namespace StackClass
