...
int cipherProcess() {
  qtConsole(
      "Введите строку, которую желаете зашифровать.\n \"Правила\" "
      "шифровки:\n - словом считается любая непрерывная последовательность "
      "символов до первого пробельного символа\n - Знаки препинания "
      "отсутсвуют в строке, иначе они буду обрабатываться вместе сос "
      "словами или как отдельные слова\n --> ");
  ...
  qtConsole("Зашифрованная строка: ");
  std::cout << resultWord;
  qtConsole("\n Ключ к строке: ");
...
// выражения (которая будет являться ключом)
std::string generateKey(const std::string& str) {
  /*
  0 - число;
  1 - знак низкого приоритета (по типу "+" или "-");
  2 - знак высокого приоритета (по типу "*" или "/");
  3 - стоп знак (для каждого ключа в конце стоит стоп-знак, для того, чтобы
  можно было склеивать ключи, а => и слова, иначе при склейке будет
  неопределенность, ибо ключ может кончаться и на 0, и на 1, и на 2, и х*й
  пойми, где че) 4,5 - скобки; в самом зашифрованном ключе присутсвовать не
  будут, но можно использовать для разнообразной постфиксной записи
  */

  // Кол-во букв в слове
  int SZ = str.size();
  // Флаги информации о предыдущих символах. Если bktFlag (скобка) true, то есть
  // открытая скобка и ее надо открыть После закрытия очередной пары скобок он
  // снова false
  ...
  if (convert.size() > 2) {
    ...
    int tmpI = 0;
    while (tmpSize > 2) {
      // Тут вообще можно зафигачить какую-то мудреную формулу с разными
      // зависимостями от типа знаков, оставшегося кол-ва цифр и тд., но мне
      // влом
      int chance = rand() % 2;
      // "Размер" скобок
      int format = 0;
...
void convertationKey(const std::string& key,
  ...
  if (word.size() % 2)
    ...
    SZ = key.size() - 2;  // Последний символ - операнд, который добавим в
                          // начало шифра, + стоп-знак
  ...
  while (iKey != SZ) {
    // 1. Если токен является операндом, то добавить его эквивалент (символ) из
    // слова в конец зашифрованного слова
    if (key[iKey] == '0') {
      ...
      StackClass::elem_of_stack now(
          4);  // Так как в зашифрованной строке не будет скобок, => ей в
               // соотвествие не ставится ничего из строки
      opStack += now;
    } else
        // 3. Если токен является правой скобкой, то выталкивать элементы из
        // opStack пока не будет найдена соответствующая левая скобка. Каждый
        // оператор добавлять в конец зашифрованных строк
        if (key[iKey] == '5') {
      ...
      while (now.get_key() != 4) {
        ...
        if (now.get_key() != 4) {
          cipherWord +=
              now.get_string()[0];  // Костыль на костыле костылем погоняет
          cipherKey += now.get_key() + 48;
        }
      }
    }
    // 4. Если токен является оператором высокого или низкого приоритетов, то
    // поместить его в opStack. Однако перед этим вытолкнуть любой из
    // операторов, уже находящихся в opStack, если он имеет больший или равный
    // приоритет, и добавить его в результирующие строки.
    else {
      if (!opStack.empty() &&
          key[iKey] <= opStack.show_first().get_key() + 48 &&
          opStack.show_first().get_key() < 3) {
        StackClass::elem_of_stack now(-1);
        opStack(now);
        if (now.get_key() == 4)
          break;
  ...
  // Проверяем стек на наличие операндов; все операнды, которые находятся в нем,
  // помещаем в конец шифров
  if (!opStack.empty()) {
    while (!opStack.empty()) {
      StackClass::elem_of_stack now;
      opStack(now);
...
std::string quaToHex(std::string& key) {
  ...
  // Если в строке остался еще один не переведенный стоп-знак, то переводим его
  // как 30
  if (!key.empty())
    result += 'C';
  return result;
...
void hexToQua(std::string& key) {
  std::string qua;
  std::string val[16] = {"00", "01", "02", "03", "10", "11", "12", "13",
...
void decipherWord(const std::string& key, std::string& word) {
  ...
  StackClass::elem_of_stack now;
  char tmpSymbol = 0;
  // Проверяем, четно слово, или нет; если четно, то ставим первый символ в
  // конец
  if (!(SZ % 2)) {
    tmpSymbol = word[0];
    word.erase(0, 1);
  ...
  while (iWord < SZ) {
    ...
    // 2. Либо токен оператор, => нуждается в двух операторах, которые должны
    // находиться в opStack
    else {
...
int decipherProcess() {
  qtConsole(
      "Внимание! В зашифрованной строке/ключе НЕ МОЖЕТ БЫТЬ "
      "пробельных символов! Все, что стоит после них, автоматически "
      "будет игнорироваться!\nВведите зашифрованную строку\n --> ");
  std::string decipherWord;
  inputType(decipherWord, std::cin);
  qtConsole("Введите зашифрованный ключ\n --> ");
