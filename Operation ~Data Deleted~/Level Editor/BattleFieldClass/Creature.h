...
namespace XCom {
	...
	private:
		...
		/* Виртуальный метод для установки дамага криссалида. Помещая его в private область, мы делаем так, что его НЕЛЬЗЯ
		вызывать из базового класса как метод, но его МОЖНО переопределить для какого-то конкретного производного класса. 
		То есть он будет переопределен только для класса Криссалид, а остальнын производные классы иметь такого метода
		не будут. => обращаясь по указателю/ссылке к экземпляру класса Криссалид, у нас будет метод set_damage, а для 
		экземпляров других производных классов метода set_damage не будет.
		*/
		virtual Creature& set_damage(int) { return *this; };
